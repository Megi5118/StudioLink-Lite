...
# bridge.py
# ──────────────────────────────────────────────────────────────────────────
#  StudioLink Lite Bridge
#  Local WebSocket <-> Roblox Studio MCP server.
#  The browser extension talks to this over ws://127.0.0.1:<PORT>.
#
#  The bridge exposes only the Roblox Studio StudioMCP process declared under
#  the fixed `roblox` entry in config.json.
#
...
#   - Tool calls are serialized safely against StudioMCP.
...
HOST = "127.0.0.1"
# Keep in sync with the extension manifest "version" - printed at
# startup so a user's terminal output alone tells us which build they're on.
BRIDGE_VERSION = "1.6.0"
PORT = int(os.environ.get("SLL_BRIDGE_PORT", os.environ.get("ZS_BRIDGE_PORT", "17613")))
HERE = os.path.dirname(os.path.abspath(__file__))
CONFIG_PATH = os.path.join(HERE, "config.json")
MAX_BROWSER_MESSAGE_BYTES = 16 * 1024 * 1024
ALLOWED_BROWSER_MESSAGE_TYPES = {
    "ping", "studio_status", "list_tools", "call_tool", "restart_mcp",
}
EXTENSION_ORIGIN_RE = re.compile(r"^(?:chrome|edge)-extension://[a-p]{32}$")

# The primary server. It is always present, added by the installer, and can
# never be edited or removed through the extension.
...
def _reclaim_bridge_port():
    ...
    if "bridge.py" not in cmdline.lower():
        log(f"port {PORT} is held by pid {pid_i} ('{name}') but it does not look "
            f"like a StudioLink Lite bridge - leaving it alone.", "yl")
        return False
    log(f"port {PORT} is held by a leftover StudioLink Lite bridge (pid {pid_i}) from a "
...
# ── local config.json read-only loader ──────────────────────────────────────
def _read_config():
        ...
        with self.start_lock:
            ...
            with _Spinner(f"    [{self.id}] starting..."):
                ...
                self._request("initialize", {
                    ...
                    "capabilities": {},
                    "clientInfo": {"name": "studiolink-lite-bridge", "version": BRIDGE_VERSION},
                }, timeout=30)
...
# ══════════════════════════════════════════════════════════════════════════
#  MANAGER  - owns the single Roblox Studio MCP client and tool index.
# ══════════════════════════════════════════════════════════════════════════
class MCPManager:
    def __init__(self):
    ...
    def load_config(self):
        ...
        spec = servers.get(PRIMARY_SERVER_ID) if isinstance(servers, dict) else None
        if not isinstance(spec, dict):
            spec = {}
        # StudioLink Lite intentionally never launches arbitrary configured
        # programs. The only allowed child is the bundled StudioMCP launcher;
        # its own SLL_STUDIO_MCP_PATH environment override selects the binary.
        configured_command = spec.get("command", "launch_studio_mcp.py")
        if configured_command != "launch_studio_mcp.py":
            log("ignored unsafe config command; using bundled launch_studio_mcp.py", "yl")
        if spec.get("args") not in (None, []):
            log("ignored StudioMCP arguments from config.json", "yl")
        self.clients = {
            PRIMARY_SERVER_ID: MCPClient(
                PRIMARY_SERVER_ID, "launch_studio_mcp.py", [])
        }
        log("configured Roblox Studio MCP server", "cy")

    def start_all(self):
        client = self.clients.get(PRIMARY_SERVER_ID)
        if client:
            client.start()
        self.rebuild_index()

    def rebuild_index(self):
        """Build the advertised Roblox Studio tool index."""
...
def probe_studio():
    ...
    if roblox is not None and roblox.is_alive() and not roblox.tools_cache:
        ...
        # transient blips) then leaves the status dot stuck green with Studio
        # fully closed. An alive client with an
...
async def broadcast_status():
    ...
    Needed because the socket starts listening before StudioMCP necessarily
    finishes launching. An extension connecting in that window gets an early,
    incomplete snapshot.
    ...
    background.js already handles a second
...
def _websocket_origin(ws):
    """Return the browser Origin across websockets 10-16 server APIs."""
...
def _allowed_websocket_client(ws):
    """Only a loopback Chromium extension may speak the bridge protocol."""
    remote = getattr(ws, "remote_address", None) or (None,)
    peer = remote[0]
    origin = _websocket_origin(ws)
    loopback = peer in ("127.0.0.1", "::1")
    extension_origin = isinstance(origin, str) and bool(EXTENSION_ORIGIN_RE.fullmatch(origin))
    return loopback and extension_origin
...
async def handler(ws):
    ...
    if not _allowed_websocket_client(ws):
        log(f"rejected WebSocket client ({peer}, origin={_websocket_origin(ws)!r})", "yl")
        await ws.close(code=1008, reason="StudioLink Lite extension required")
        return
    clients.add(ws)
    ...
    try:
        ...
        async for raw in ws:
            if not isinstance(raw, (str, bytes)) or len(raw) > MAX_BROWSER_MESSAGE_BYTES:
            ...
            if not isinstance(msg, dict):
                await ws.send(json.dumps({"type": "error", "error": "message must be an object"}))
                continue
            mtype = msg.get("type")
            rid = msg.get("id")
            if mtype not in ALLOWED_BROWSER_MESSAGE_TYPES or not _valid_request_id(rid):
                await ws.send(json.dumps({
                    "type": "error", "id": rid if _valid_request_id(rid) else None,
            ...
            if mtype == "ping":
                ...
                if not isinstance(name, str) or not name or len(name) > 256 or not isinstance(args, dict):
                    await ws.send(json.dumps({
                        "type": "tool_result", "id": rid, "ok": False,
                        "kind": "invalid_request", "error": "invalid tool request",
                    }))
                    continue
                try:
                    timeout_ms = float(msg.get("timeout", 120000))
                except (TypeError, ValueError):
                    timeout_ms = 120000
                timeout = max(1.0, min(timeout_ms / 1000.0, 120.0))
...
async def server_watch():
    """Poll StudioMCP and restart it if it dies unexpectedly (e.g. the
    ...
    # hides completely. We still keep restarting because the cause may be
    # transient, but the terminal names the problem: exit code, stderr and
            ...
            try:
                if not client.is_alive():
                    ...
                    if looping and now - client.loop_warned_at > LOOP_WARN_COOLDOWN:
                        ...
                        log(f"[{sid}] common causes: Roblox Studio is not ready, a port "
                            f"conflict, an antivirus killing StudioMCP, or a broken Studio install. "
                    ...
                    if looping and client.restart_times and now - client.restart_times[-1] < 15:
                        ...
                        # while still retrying forever if the cause clears.
                        continue
...
async def studio_watch(initial_app, initial_place=None):
    ...
    while True:
        ...
        if app is not None and app != prev_app:
            if app is True:
                # Use the primary client's current count so the status matches
                # the StudioMCP process that just attached.
...
async def main():
    print(f"\n{C['cy']}  StudioLink Lite Bridge v{BRIDGE_VERSION}{C['reset']}  {C['dim']}- Roblox Studio - ws://{HOST}:{PORT}{C['reset']}\n")
    ...
    async def _boot_and_diagnose():
        """Launch StudioMCP and print the boot diagnostic banner.

        This runs after the socket is listening so a slow or absent Roblox
        Studio never prevents the extension from connecting to the bridge.
        """
        try:
        ...
        if roblox_client is not None and (roblox_total == 0 or _st["app"] is False):
            ...
            log(f"ready {total} Roblox Studio tools available", "gr")
    ...
    async def _early_status_pushes():
        ...
        StudioMCP startup can take up to ~48s because of its own internal
        "waiting for tools" retry loop. A single broadcast placed after
        start_all() could therefore arrive too late. Poll and broadcast a few
        times so any extension connected during that window self-corrects
    ...
    try:
        ...
        if getattr(e, "errno", None) in (98, 10048) or "10048" in str(e):
            ...
            log(f"    Or set a different port before start.bat:  set SLL_BRIDGE_PORT=17614", "yl")
            return
        raise

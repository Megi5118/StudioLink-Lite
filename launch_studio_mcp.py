...
#  `SLL_STUDIO_MCP_PATH` when discovery is not enough. The legacy environment
#  name remains a fallback for existing personal setups.
# ──────────────────────────────────────────────────────────────────────────
...
ENV_OVERRIDE = "SLL_STUDIO_MCP_PATH"
LEGACY_ENV_OVERRIDE = "ZS_STUDIO_MCP_PATH"
...
def find_studio_mcp() -> Optional[Path]:
    override_name = ENV_OVERRIDE if os.environ.get(ENV_OVERRIDE) else LEGACY_ENV_OVERRIDE
    ...
    if override_value:
        ...
        sys.stderr.write(
            f"launch_studio_mcp: {override_name} is set but does not point to a valid StudioMCP binary: {override_value}\n"
        )
    if sys.platform == "darwin":

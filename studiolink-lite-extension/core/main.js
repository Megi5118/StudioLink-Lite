...
// waits for the model's reply, parses StudioLink Lite commands (ZSParse), asks the
...
(() => {
  ...
  const log = (...a) => console.log("[studiolink]", ...a);
  ...
  // console (filter "[studiolink-diag]") or window.__zsDiag (also mirrored onto a hidden
  ...
  function diag(event, data) {
    ...
    try { console.log("[studiolink-diag]", e.iso, event, JSON.stringify({ ...data, ...snap })); } catch {}
    try {
  ...
  // Version is sourced from the manifest so the UI and bridge stay aligned.
  const EXT_VERSION = chrome.runtime.getManifest().version;
  // Browser-session providers. Keep in sync with manifest.json
  // content_scripts and background.js PROVIDER_URLS when adding a provider.
  const AI_SITES = [
    { id: "deepseek", name: "DeepSeek", url: "https://chat.deepseek.com/" },
    { id: "chatgpt", name: "ChatGPT", url: "https://chatgpt.com/" },
    { id: "hy4", name: "Hy4 (WorkBuddy)", url: "https://www.workbuddy.ai/app" },
    { id: "gemini", name: "Gemini", url: "https://gemini.google.com/app" },
    { id: "kimi", name: "Kimi", url: "https://www.kimi.ai/" },
    { id: "glm", name: "GLM", url: "https://chat.z.ai/" },
    { id: "qwen", name: "Qwen", url: "https://chat.qwen.ai/" },
    { id: "arena", name: "Arena", url: "https://arena.ai/text/direct" },
  ...
  const A = {
    ...
    // gives the "screen" chip optimistically. A newly-added StudioMCP tool's
    // name may tell us nothing, but once we have seen it
  ...
  async function waitForResponse(base) {
    ...
    while (Date.now() - lastActiveAt < TIMEOUT) {
      ...
      if (ZSParse.hasToolSignature(r)) {
        ...
        // to parse - typically an unescaped quote inside a string parameter broke
        // the JSON. Unlike execute_luau there is no ###LUA### fallback, so the
      ...
      // StudioLink Lite command - DeepSeek's DSML invoke/parameter tags (reported by
  ...
  // the model "the local StudioLink Lite bridge is unreachable", which sent the user
  ...
  // Any StudioMCP tool result carrying images is also checked generically at
  // the point results are handled, so future image-returning commands remain
  // safe on text-only providers without guessing names here.
  ...
  // Compare qualified and bare names for compatibility with persisted turns
  // created by older bridge versions.
  ...
  // ZS.toolCategory. A newly-added tool's name may reveal nothing, so we learn
  ...
  // Refresh the Roblox Studio tool catalogue, but never pay for the same round
  // trip twice during one short command sequence.
  ...
  async function runTool(call) {
    ...
    // Virtual command: list available Roblox Studio commands with full details.
    if (name === "list_commands" || name === "list_tools") {
      ...
      // actually unusable, return a clear offline result instead of firing tools.
      const s = A.bridge || {};
      const srv = s.servers || [];
      const rbx = srv.find((x) => x.id === "roblox");
      const rbxAlive = rbx ? !!rbx.alive : !!s.mcpAlive;
      const rbxUsable = !!s.connected && rbxAlive && s.studio !== false;
      if (!rbxUsable) {
        return `Output of '${name}':\nRoblox Studio is currently OFFLINE (closed, no place open, or its MCP server disabled), so its commands cannot run. This is an environment problem on the user's machine, not your mistake. Tell the user in one short sentence to open their place in Roblox Studio and enable its MCP server, then wait.`;
      }
      ...
      return `Output of '${name}':\nRoblox Studio commands (${A.toolList.length}):\n\n${lines.join("\n\n")}`;
    }
    ...
    if (r.ok) {
      if (r.images && r.images.length && !P.supportsVision) {
        // Any StudioMCP tool can turn out to return images -
      ...
      if (r.images && r.images.length) {
        // Show the capture in a left-hand StudioLink Lite popup (from the in-memory
    ...
    if (r.kind === "stale-extension") {
      ui.banner("warn", "Reload this page",
        "StudioLink Lite was updated or reloaded while this tab was open, so this page is running an " +
  ...
  // error as its summary. Treat a result whose FIRST
    ...
    try {
      while (!A.stop) {
        ...
        if (res.kind === "tool") {
          ...
          // Trace the chip's derived phase vs summary. A tool whose output leads
          // with a warning/diagnostic line may otherwise resolve ✓ done - the
          ...
          // Any StudioMCP tool that actually returned an image becomes a
          // "screen" chip, even if its name did not let us predict that.
  ...
  function withSysResend(text) {
    ...
    const rider =
      "\n\n────────────────────────────────\n" +
      ZS.RESEND_MARKER + "\n" +
      "(System note from StudioLink Lite - an automatic re-statement of your operating " +
      ...
      "commands. To be explicit: the StudioLink Lite extension IS running in this page right " +
  ...
  async function startSession() {
    ...
    try {
      await ensureTools(true); // boot with a fresh catalogue; the TTL covers the
                               // list_commands call the model makes seconds later
      if (!alive()) return;
  ...
  const decorate = {
    ...
    classify(item, next) {
      ...
      // ONLY in a real StudioLink Lite session (started or bootstrapping). Without
      ...
      if (P.isAssistantItem(item) && ZSParse.hasCommandShape(txt) &&
        ...
        // loop paints its own chip - most visible on the instant list_commands
        // virtual command. Keep it spinning instead; staleLastOrphan
        ...
        if (phase === "done" && next && P.isUserItem(next)) {
          ...
          // test would miss), so a revisited conversation
  ...
  // ════════════════════════════════════════════════════════════════════════
  //  UI  (control panel, stop button, banners, toast, input cover)
  // ════════════════════════════════════════════════════════════════════════
  const ui = (() => {
    ...
    let bridgeOk = false, studioDown = false, placeDown = false, appDown = false, studioProcUp = false;
    ...
    function build() {
      ...
      // menu (provider and custom prompt). No floating panel,
      // no overlay on the input - the composer stays fully usable for plain chat.
      root.innerHTML = `
        <div id="zs-bar">
          <span id="zs-dot" class="off" title=""></span>
          <span id="zs-brand">StudioLink Lite <span class="zs-free">v${EXT_VERSION}</span></span>
          <span id="zs-state"></span>
          <button id="zs-action"></button>
          <button id="zs-stop" hidden>■ Stop</button>
          <button id="zs-switch" aria-label="Provider and settings" title="Provider and settings"><span id="zs-switch-name"></span><svg width="11" height="11" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><polyline points="6 9 12 15 18 9"/></svg></button>
        </div>
      ...
      const toggleMenu = () => {
        ...
        if (!menuEl.hidden) {
          ...
          buildMenu();
          syncMenuPrompt();
          // On a fresh open, menuEl has no max-height yet - that is applied by
          // placeBar() on the next animation frame. Defer the scroll reset until
          // the panel has its final clipped geometry.
      ...
      openMenuFn = () => { if (menuEl.hidden) toggleMenu(); };
      document.addEventListener("click", (e) => {
        if (menuEl.hidden) return;
        if (!menuEl.contains(e.target) && !switchBtn.contains(e.target))
          menuEl.hidden = true;
      }, true);
    ...
    // Minimal private-development menu. Process configuration deliberately stays
    // local to config.json and is never accepted from a webpage.
    function buildMenu() {
      const hostOf = (url) => { try { return new URL(url).hostname.replace(/^www\./, ""); } catch { return ""; } };
      let sites = "";
      for (const provider of AI_SITES) {
        const current = provider.id === P.id;
        const label = `<span class="zs-site-name"><span>${provider.name}</span><span class="zs-site-host">${hostOf(provider.url)}</span></span>`;
        sites += current
          ? `<div class="zs-site-opt zs-site-here">${label}<span class="zs-site-badge">active</span></div>`
          : `<button class="zs-site-opt" data-u="${provider.url}">${label}<span class="zs-site-go">&rarr;</span></button>`;
      }
      const bridgeText = A.bridge && A.bridge.connected ? "Connected" : "Disconnected";
      const studioText = bridgeOk ? "Connected" : (A.bridge && A.bridge.connected ? "Connecting" : "Disconnected");
      const agentText = A.running || A.starting || A.injecting ? "Working" : (A.started ? "Active" : "Idle");
      menuEl.innerHTML =
        `<div class="zs-menu-head"><span class="zs-menu-logo">StudioLink Lite</span><span class="zs-menu-tag">v${EXT_VERSION}</span></div>
         <section class="zs-menu-sec">
           <div class="zs-sec-label"><span>Status</span></div>
           <div class="zs-menu-note">Bridge: <b>${bridgeText}</b><br>Roblox Studio: <b>${studioText}</b><br>Agent: <b>${agentText}</b></div>
         </section>
         <section class="zs-menu-sec">
           <div class="zs-sec-label"><span>Provider</span></div>
           ${sites}
         </section>
         <section class="zs-menu-sec">
           <div class="zs-sec-label"><span>Custom prompt</span></div>
           <div class="zs-menu-note">Appended below the built-in prompt when a new session starts.</div>
           <textarea id="zs-set-text" rows="4" placeholder="Optional project-specific instructions"></textarea>
           <div class="zs-set-row"><button id="zs-set-save">Save</button><span id="zs-set-status"></span></div>
         </section>`;
      menuEl.querySelectorAll("button.zs-site-opt").forEach((button) =>
        button.addEventListener("click", () => {
          try { window.open(button.dataset.u, "_blank", "noopener"); } catch {}
          menuEl.hidden = true;
        }));
      const textarea = menuEl.querySelector("#zs-set-text");
      const status = menuEl.querySelector("#zs-set-status");
      textarea.value = customPrompt;
      menuEl.querySelector("#zs-set-save").addEventListener("click", () => {
    ...
    function renderBar() {
      ...
      // chats share a key, and the conversation id only appears mid-bootstrap).
        ...
        // Prefer the filtered command catalogue. After a page reload it is empty
        // until the next list_tools, so fall back to Roblox's reported health.
        const health = A.bridge && (A.bridge.servers || []).find((x) => x.id === "roblox");
        const tools = A.toolList.length || (health && health.tools) || (A.bridge && A.bridge.tools) || 0;
      ...
      // The provider/settings control remains available in every state.
    ...
    function setStatus(s) {
      ...
      // StudioLink Lite status tracks only the Roblox MCP server. Old bridges do
      // not send per-server health, so fall back to their aggregate Roblox fields.
      ...
      else txt = `Connected · ${robloxTools} tools ready`;
    ...
    function staleExtensionAlert() {
      ...
      b.innerHTML = `<div class="zs-banner-t">⚠ Reload this page to reconnect StudioLink Lite</div>
        <div class="zs-banner-m">StudioLink Lite was updated or reloaded while this tab was open, so commands can no longer run from this copy. The bridge and Roblox Studio are fine.</div>
    ...
    function bridgeAlert(on) {
      ...
      b.innerHTML = `<div class="zs-banner-t">⚠ Bridge disconnected</div>
        <div class="zs-banner-m">Restart the StudioLink Lite bridge and keep Roblox Studio open. The connection will recover automatically.</div>
    ...
    // Left-hand StudioLink Lite popup showing the latest screen_capture. Fed from the
    ...
    build();
    return { setStatus, staleExtensionAlert, setStarted, setStarting, showStop, markStopping, inputCover, toast, banner, showImages, nudgeStart, updateStartGate, getCustomPrompt, openMenu: () => openMenuFn && openMenuFn() };
  })();
  ...
  // StudioLink Lite's own UI). A genuine "regenerate ↻" is always such a click;
  ...
  setInterval(() => {
    ...
    // The bare isHardGenerating() term is gated on a live StudioLink Lite session: on
  ...
  function currentSessionStatus() {
    ...
    return {
      ...
      state: A.stopping ? "stopping" : A.starting ? "connecting" :
        A.running || A.injecting ? "working" : A.started ? "active" : "idle",
    };
  }
  ...
  chrome.runtime.onMessage.addListener((msg, _sender, sendResponse) => {
    ...
    } else if (msg && msg.type === "zs-open-menu") {
      ui.openMenu();
    } else if (msg && msg.type === "sll-session-status") {
      sendResponse(currentSessionStatus());
    } else if (msg && msg.type === "sll-start-session") {
  ...
  // A conversation IS a StudioLink Lite session if any rendered turn carries a
  // telltale artefact: the system-prompt marker, an injected tool-result /
  // system-note turn, or a StudioLink Lite command an assistant wrote. Works even
  ...
  function syncSessionState() {
    ...
    // via the SITE's own new-chat (not StudioLink Lite's button), the loop is bound to
  ...
  log(`StudioLink Lite content script ready (provider: ${P.id})`);
})();

...
const ZS = (() => {
  ...
  const APP_NAME = "StudioLink Lite";
  const SYS_MARKER = "⟦ZS-SYS⟧";
  const LEGACY_MEMORY_PATH = "game.ServerStorage.Zero" + "Script.Memory";
  ...
  const FEEDBACK = {
    ...
    parseError: (reason, toolName) => {
      ...
      const notes = {
        malformed:
          "ERROR: a StudioLink Lite command was detected in your reply but its JSON could not be parsed. " +
          ...
          "Please retry.",
        unclosed:
          "ERROR: your StudioLink Lite command was cut off before it finished - the JSON object" +
        ...
        // "studio_id": "..."}) instead of StudioLink Lite's envelope. Seen live on
        ...
        dsml:
          ...
          "tags). StudioLink Lite cannot read that format, so the command did not run. Never use those tags " +
    ...
    studioOffline:
      ...
      "the MCP server (Assistant settings), then stop until they confirm it is back.",
    ...
    staleExtension:
      "ERROR: the StudioLink Lite extension was reloaded or updated while this page was open, so this " +
    ...
    bridgeOffline:
      "ERROR: the local StudioLink Lite bridge is unreachable, so no command could run. " +
  ...
  function buildSystemPrompt(opts = {}) {
    ...
    const prompt = `CONTEXT: the user has installed a browser extension called StudioLink Lite in their own browser. Here is how it works, so you can use it on their behalf:
A browser extension (StudioLink Lite) is running inside this page. It watches your replies. When it detects a StudioLink Lite command in your text, it runs it against the locally connected Roblox Studio MCP server and sends the result back as the next message. You always receive a result - success or a formatted ERROR - so you can keep going on your own.

The user's open Roblox Studio place is reached through the local bridge. Call \`list_commands\` FIRST for the exact Roblox Studio commands and their full parameter details. You do not need any special capability yourself - you write a StudioLink Lite command as ordinary reply text and the extension does the rest.

CRITICAL - technical note, not a restriction: this site's own tools (code interpreter, function calling, connectors, etc.) run in a separate sandbox that has no access to the user's Roblox Studio - so calling them wouldn't reach the user's project at all. StudioLink Lite commands are different: they are plain JSON typed directly into your normal reply text, which this extension reads and executes against the local StudioMCP server. So for anything involving the user's project, write the JSON below as ordinary text instead of calling a function - that's the only channel that reaches their machine. (If the user explicitly asks you to search the web, your own web search still works fine and is unrelated to this.) Internal reasoning (deep-think modes) is fine.
...
The ModuleScript at game.ServerStorage.StudioLinkLite.Memory is your long-term memory for this project, saved inside the place. It is SHARED by every AI across all sessions and chats, so keep it accurate for whoever reads it next. Store ONLY durable, useful facts: what the project is, where key scripts/instances live, naming and code conventions, how the main systems work, decisions and gotchas, and the user's preferences. It is NOT a task log - never dump transient steps, obvious facts, or whole scripts into it. Keep it short.

- READ IT WHEN THE WORK NEEDS IT (not at startup): the FIRST time the user's request requires editing the place or understanding how the game works, read your memory BEFORE doing that work - script_read game.ServerStorage.StudioLinkLite.Memory. Skip it for pure chit-chat or questions unrelated to the project. If the new memory does not exist, check the pre-fork compatibility path ${LEGACY_MEMORY_PATH} once. When legacy memory exists, copy its complete current contents into game.ServerStorage.StudioLinkLite.Memory and use only the new path afterward. If neither exists, create the new memory with multi_edit (className "ModuleScript", first edit with old_string "") using exactly this skeleton (multi_edit auto-creates the StudioLinkLite folder):
${BT}
return [==[
# Project memory
## Overview
...
IMPORTANT: Your very first action is to write \`list_commands\` with no params to get the full Roblox Studio command reference with parameter details - never guess a command name or parameter that wasn't in that result. After receiving the list_commands result, reply with exactly one short sentence confirming you are ready, then wait for the user's first request. (Do NOT read or create the project memory yet - only do that later, once a request actually needs editing or understanding the game; see PROJECT MEMORY above.) If that first list_commands, or any later command, reports Studio offline, tell the user in one short sentence to open their place and enable Studio's MCP server, then stop and wait.`;
  ...
  const TOOL_NOTES = {
    ...
    user_keyboard_input:
      ...
      "(StudioLink Lite auto-fills datamodel_type:\"Client\" if you omit it, but the game must still be running.) " +
  ...
  // StudioLink Lite reminder (NOT a user message and NOT a new command to run).
  function toolsReminder(tools) {
    ...
    return (
      "\n\n────────────────────────────────\n" +
      "(System note from StudioLink Lite - this is an automatic REMINDER, not a request and not a new result. " +
  ...
  function memoryNudge() {
    return (
      ...
      "game.ServerStorage.StudioLinkLite.Memory with multi_edit - only useful, lasting facts. If nothing changed, ignore this.)"
    );
  }

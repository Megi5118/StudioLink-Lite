...
const ZSProvider = (() => {
  ...
  // Scope to the SITE's composer only: never match StudioLink Lite's own injected
  ...
  const getEditor = () => {
    ...
    // returned it - and barMount() then dragged the whole StudioLink Lite bar INTO the
  ...
  function enforceComposer(reason) {
    ...
    // off) without StudioLink Lite reverting their choice every frame.
  ...
  function truncateForSend(text) {
    ...
    const marker =
      `\n\n[…StudioLink Lite: result truncated to fit DeepSeek's ${SEND_CAP}-character ` +
    ...
    try {
      return [...document.querySelectorAll("img")].filter(
        (im) => !im.closest(S.chatItem) &&
          // blob: = pending local preview; the alt (our "studiolink_..." filename)
          ...
          (/^blob:/.test(im.getAttribute("src") || "") || /^studiolink_/.test(im.getAttribute("alt") || "")));
    } catch { return []; }
  };
  ...
  //   isStarted():bool        - a StudioLink Lite session exists in this chat

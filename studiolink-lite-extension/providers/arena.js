...
//    CodeMirror/Monaco), wrapped in a <div class="not-prose">. StudioLink Lite's
...
const ZSProvider = (() => {
  ...
  // textareas are recaptcha/aria-hidden). Scope away StudioLink Lite's own settings
  ...
  function truncateForSend(text) {
    ...
    const marker =
      `\n\n[…StudioLink Lite: result truncated to fit Arena's input limit - ` +
  ...
  // StudioLink Lite only supports DIRECT. Battle / Side by Side force a fresh A/B
  ...
  // bounces the tab), which StudioLink Lite can't drive. We nudge the mode dropdown
  ...
  function modeWarning() {
    ...
    if (um)
      return `Turn off <b>${um.label}</b> (${um.tip} in the composer) - ` +
        `StudioLink Lite only works in plain chat. ${um.label} mode uses a different ` +
    ...
    return `Switch the mode dropdown to <b>Direct</b> - StudioLink Lite only works in ` +
  ...
  // StudioLink Lite bar out of the way: the anchored bar is transparent but still a
  ...
  // up, the anchored StudioLink Lite bar (a real, full-width element hugging the

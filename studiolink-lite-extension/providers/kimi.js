...
const ZSProvider = (() => {
  ...
  const RE = {
    ...
    // on normal answers that tell the user to try again. Require the site's
    // specific "please try again" / "réessayer plus tard" phrasing.
  ...
  // Scope to the SITE's composer only: skip StudioLink Lite's own injected UI (the
  ...
  const getEditor = () => {
    ...
    // barAnchor() then hugged the fixed StudioLink Lite bar over the editor. The real
  ...
  // its built-in tools over the StudioLink Lite command protocol. On K3 the only
  ...
  function nativeAgentModeOn() {
    ...
    // its own tooling instead of emitting StudioLink Lite command blocks.
    return !!(m && /swarm|agent/i.test(m.textContent || ""));
  }
  // Visible mode guard for the StudioLink Lite bar (core renderBar reads this every
  ...
  function modeWarning() {
    if (nativeAgentModeOn())
      return `Switch the model picker off <b>K3 Swarm</b> (pick <b>K3</b> or <b>Instantané</b>) - ` +
        `Kimi's own agentic mode replaces the StudioLink Lite commands with its native tools and breaks the agent loop.`;
    return "";
  }
  ...
  // Pick a model that actually speaks the StudioLink Lite command protocol, ONCE,
  ...
  return {
    ...
    // the capture is shown in the core's left-hand StudioLink Lite popup (ui.showImages).
    supportsVision: true,
    timings,
    ...
    reliableCounts: true,
    // Shown as a permanent, non-intrusive notice in the StudioLink Lite panel.
    // Kimi sometimes reaches for its OWN built-in/native tools (web search, code
    // runner, etc.) instead of emitting the StudioLink Lite command blocks that drive
    ...
    unstableWarning:
      ...
      "If it stops acting in Roblox Studio and answers in plain text or runs its own tools, remind it to use the StudioLink Lite commands - or start a new session.",
